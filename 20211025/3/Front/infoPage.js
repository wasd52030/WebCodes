...
export function Info() {
    let InfoPageStr = ''
    axios.get("http://localhost/20211025/3/Back/index.php?action=getUsers")
        .then(res => {
            switch (res['status']) {
                case 200:
                    let data = res['data']['result']
                    InfoPageStr = `<table>`;
                    InfoPageStr += `<tr><td>員工編號</td><td>密碼</td><td>email</td><td>電話</td><td><button id='add'>新增使用者</button></td></tr>`;
