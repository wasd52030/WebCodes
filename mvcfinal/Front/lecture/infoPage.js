...
export function lectureInfo(id) {
    Request().post("index.php?action=getlectureList", Qs.stringify({ teacherid: id }))
        .then(res => {
            switch (res['status']) {
                case 200:
                    ...
                    if (data.length == 0) {
                        ...
                        page += `<table border='1'>`
                        page += `<tr><td>編號</td><td>課程編號</td><td>課程名稱</td><td>檔案連結</td></tr>`
